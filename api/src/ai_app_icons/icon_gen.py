...
Colors:
- Re-read the app description above and identify every color the user \
mentioned (hex codes like #FF3B30, named colors like "cobalt blue", "mint \
green", "hot pink", "black", or "white"). Those colors MUST appear in the icon, matched as \
closely as possible. Do not substitute them for a "better" or more \
harmonious palette — match the exact hues the user asked for, even if \
they clash with your instincts. If no color is specified, pick a palette \
that fits the app concept and the visual style below. If the user specified "all" or "fully" or "completely" a color, \
use ONLY that color.
...
Colors: re-read the instruction above. If it names any specific color \
(hex code, RGB value, or a named color like "cobalt blue", "mint green"), \
the output MUST use that exact color on the element the instruction points \
at. Do not substitute or "harmonize" the requested color.
